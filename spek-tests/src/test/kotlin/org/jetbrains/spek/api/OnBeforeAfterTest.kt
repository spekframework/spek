...
public class OnBeforeAfterTest {
    ...
    @Test
    @Parameters(source = SpekTestRunnerProvider::class)
    public fun callOnBefore(runner: SpekTestRunner) {
        val log = arrayListOf<String>()
        runner.runTest(object : TestSpek() {
            init {
                given("a") {
    ...
    @Test
    @Parameters(source = SpekTestRunnerProvider::class)
    public fun callOnAfter(runner: SpekTestRunner) {
        val log = arrayListOf<String>()
        runner.runTest(object : TestSpek() {
            init {
                given("a") {
    ...
    @Test
    @Parameters(source = SpekTestRunnerProvider::class)
    public fun callOnBeforeAndOnAfter(runner: SpekTestRunner) {
        val log = arrayListOf<String>()
        runner.runTest(object : TestSpek() {
            init {
                given("1") {
